...
from ambulance_system.models import TrafficConditionLog,Location,ResponsibleUnit,TrafficCondition,Ambulance,AmbulanceSchedule
...
def adminmain(request):
	...
	if search == 1:
		...
		car_accidents = TrafficConditionLog.objects.filter(Location__in=locations)

		locations = Location.objects.filter(CountryName__contains=location_name2)
		
		car_accidents_all = TrafficConditionLog.objects.filter(Location__in=locations)
		...
		car_accidents_all = TrafficConditionLog.objects.all().order_by('Location')
...
def index(request):
	...
	car_accidents_location=TrafficConditionLog.objects.all().order_by('Location')
...
def table(request):
	...
	if request.method == 'POST':
		...
		try:
			...
			location=Location.objects.create(Name=location_name,Longitude=unit.Location.Longitude,Latitude=unit.Location.Latitude,CountryName=unit.Location.CountryName)
			location.save()
		...
		log=TrafficConditionLog.objects.create(Location=location, TrafficCondition=condition, Datetime=time, ResponsibleUnit=unit, Solved=False, Processed=process)
		log.save()
